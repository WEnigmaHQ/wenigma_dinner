...
export default function Fashion(apiurl: any) {
  ...
  padding: 10,
                    ...
                    {fashionArticles.length<= 0 ? <Text variant={'headlineLarge'} style={{color: 'black', top: 300, left: 50}}> {fashionArticles.length} results in 1 sec .... </Text> : <FlatList data={fashionArticles} renderItem={({item}) =>
                        <Card>
                            <Card.Title title={item.title} subtitle={`${fashionArticles.length}` + 'results loaded in 1 sec ...'} left={LeftContent}>
                            </Card.Title>
                            <Card.Content>
