...
export default function Sports(apiurl: any) {

    // handle sport activities news
  ...
  padding: 10,
                    ...
                    {SportsArticles.length<= 0 ? <Text variant={'headlineLarge'} style={{color: 'black', top: 300, left: 50}}> {SportsArticles.length} results in 1 sec .... </Text> : <FlatList data={SportsArticles} renderItem={({item}) =>
                        <Card>
                            <Card.Title title={item.title} subtitle={`${SportsArticles.length}` + 'results loaded in 1 sec ...'} left={LeftContent}>
                            </Card.Title>
                            <Card.Content>
